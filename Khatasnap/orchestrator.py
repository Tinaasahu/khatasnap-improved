...
def process_invoice(file_bytes: bytes, filename: str) -> dict:
    ...
    # Include raw OCR blocks for downstream lightweight post-processing
    # (e.g. catalog mapping when table extraction fails).
    try:
        result["ocr_blocks"] = [
            {
...
def _fix_vendor_name(data: dict) -> str:
    sup = data.get("supplier_details") or {}
    inv = data.get("invoice_details") or {}
    vn = str(sup.get("name", "") or data.get("vendor_name", "")).strip()
    inv_no = str(inv.get("invoice_number", "") or data.get("invoice_no", "")).strip()
    # If vendor_name looks like invoice number and invoice_no is empty, clear it
    if _looks_like_invoice_no(vn) and not inv_no:
        return ""
    return vn
...
def _fix_invoice_no(data: dict) -> str:
    ...
    if not inv_no and _looks_like_invoice_no(vn):
        return vn
    return inv_no
...
def _assemble_from_vlm(vlm_data_list: list, all_blocks: list) -> dict:
    ...
    invoice_det = data.get("invoice_details") or {}
    ...
    for item in raw_items:
        ...
        if not name: name = str(item.get("raw_text", "")).strip()
        
        qty    = f(item.get("quantity", item.get("qty", 0)))
        rate   = f(item.get("price", item.get("rate", 0)))
        
        # New prompt might not have `amount`; calculate it or default 0
        amount = f(item.get("amount", round(qty * rate, 2) if qty and rate else 0))
    ...
    result = {
        # Metadata specific to new prompt
        "bill_type":      data.get("bill_type", "UNKNOWN"),
        "needs_user_selection": data.get("needs_user_selection", False),
        
        # Fix: if vendor_name looks like an invoice number, swap them
        "vendor_name":    _fix_vendor_name(data),
        "gstin":          str(supplier.get("gst", data.get("gstin", ""))).strip(),
        "phone":          str(data.get("phone", "")).strip(),
        "invoice_no":     _fix_invoice_no(data),
        "invoice_date":   str(invoice_det.get("date", data.get("invoice_date", ""))).strip(),
